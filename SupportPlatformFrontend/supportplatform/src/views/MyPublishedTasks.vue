<template>
  <div class="my-published-tasks-container" v-loading="loading">
    <el-card v-if="error" class="error-card">
      ...
      <el-alert
        title="Failed to load your published tasks"
        type="error"
        :description="error"
        show-icon
        :closable="false"
      />
      <el-button @click="fetchMyPublishedTasks" type="primary" style="margin-top: 20px;">Retry</el-button>
    </el-card>
    ...
    <el-card v-else>
      ...
      <el-table :data="tasks" style="width: 100%" empty-text="您还没有发布任何任务">
    ...
    <el-dialog
      ...
      center
    >
      <div v-if="paymentLoading" v-loading="paymentLoading" element-loading-text="正在生成支付二维码..." style="height: 260px;"></div>
      <div v-else style="text-align: center;">
        <p>请使用支付宝扫码完成支付</p>
        <vue-qrcode-component :text="qrCodeUrl" :size="200" />
        <p>任务赏金: <strong>¥ {{ selectedOrderDetail.reward ? selectedOrderDetail.reward.toFixed(2) : 'N/A' }}</strong></p>
      </div>
       <template #footer>
        <span class="dialog-footer">
          <el-button @click="handlePaymentDialogClose">关闭</el-button>
        </span>
      </template>
    </el-dialog>
      ...
      <el-form :model="reportForm" label-width="120px">
        <el-form-item label="举报类型">
          <el-radio-group v-model="reportForm.reportType">
            <el-radio label="USER">举报接单者</el-radio>
        ...
        <el-form-item v-if="reportForm.reportType === 'USER'" label="举报对象">
          <p>接单者: {{ selectedOrderDetail.receiverUsername }} (ID: {{ selectedOrderDetail.receiverId }})</p>
        </el-form-item>
    ...
    >
      <el-form :model="reviewForm" label-width="120px">
        <el-form-item label="评价接单者">
            <p>{{ selectedOrderDetail.receiverUsername }} (ID: {{ selectedOrderDetail.receiverId }})</p>
        </el-form-item>
...
import { ElMessage, ElTable, ElTableColumn, ElCard, ElTag, ElButton, ElAlert, ElDialog, ElForm, ElInput, ElDivider, ElRate, ElRadioGroup, ElRadio, ElSelect, ElOption } from 'element-plus';
...
const getOrderStatusText = (status) => {
  switch (status) {
    ...
    case 'APPLIED': return '已申请'; // Might not be typical for 'my published' unless it shows applicant counts
    case 'ACCEPTED': return '已接受';
    case 'SUBMITTED': return '已提交'; // Submitted by the assignee
    case 'CONFIRMED': return '已确认'; // Confirmed by the publisher (me)
...
const fetchMyPublishedTasks = async () => {
  loading.value = true;
  error.value = null;
  try {
    const response = await axios.get('/api/orders/my_published');
    ...
    error.value = err.message || 'An unknown error occurred while fetching your tasks.';
    ElMessage.error('获取我发布的任务列表失败: ' + error.value);
    tasks.value = [];
  } finally {
    loading.value = false;
  }
...
const formatDate = (dateString) => {
  ...
  // Replace space with 'T' for better browser compatibility as ISO 8601 format is more reliable
...
const canConfirm = computed(() => {
  return selectedOrderDetail.value && selectedOrderDetail.value.orderStatus === 'SUBMITTED';
});

const canPay = computed(() => {
    if (!selectedOrderDetail.value) return false;
    return selectedOrderDetail.value.orderStatus === 'CONFIRMED' && selectedOrderDetail.value.paymentStatus === 'UNPAID';
});

const canCancel = computed(() => {
    if (!selectedOrderDetail.value) return false;
    const status = selectedOrderDetail.value.orderStatus;
    return status === 'UNASSIGNED' || status === 'ACCEPTED';
});

const canReport = computed(() => {
    if (!selectedOrderDetail.value || !userInfo.value || !selectedOrderDetail.value.receiverId) {
        return false;
    }
    // Publisher (current user) cannot report themselves (if they are also the receiver somehow)
    return selectedOrderDetail.value.receiverId !== userInfo.value.userId;
});

const canReview = computed(() => {
    if (!selectedOrderDetail.value || !userInfo.value) return false;
    // Can only review if the task is confirmed
...
const reportableReviews = computed(() => {
    ...
    // Publisher can report reviews written about them.
...
const confirmOrderHandler = async () => {
    ...
    try {
        const response = await axios.post('/api/orders/confirm', { orderId: selectedOrderDetail.value.orderId });
...
const cancelOrderHandler = async () => {
    ...
    actionLoading.value = true;
    try {
        const response = await axios.post('/api/orders/cancel', { orderId: selectedOrderDetail.value.orderId });
        if (response.data.code === 200) {
...
const paymentHandler = async () => {
    ...
    try {
        const response = await axios.post('/api/payment/pay', { orderId: selectedOrderDetail.value.orderId });
        if (response.data.code === 200 && response.data.data) {
            qrCodeUrl.value = response.data.data;
            startPollingPaymentStatus(selectedOrderDetail.value.orderId);
        } else {
            ElMessage.error(`支付发起失败: ${response.data.msg}`);
...
const startPollingPaymentStatus = (orderId) => {
    ...
    pollingInterval = setInterval(async () => {
        if (attempts >= maxAttempts) {
            stopPolling();
            ElMessage.warning('支付超时，请重试。');
            handlePaymentDialogClose();
            return;
        }

        try {
            const response = await axios.post('/api/orders/detail', { orderId });
            if (response.data.code === 200 && response.data.data) {
                if (response.data.data.paymentStatus === 'PAID') {
                    ...
                    paymentDialogVisible.value = false;
                    dialogVisible.value = false;
                    fetchMyPublishedTasks(); // Refresh the main task list
                }
            }
        } catch (error) {
...
const submitReportHandler = async () => {
  ...
  try {
    ...
    if (payload.reportType === 'USER') {
      if (!selectedOrderDetail.value || !selectedOrderDetail.value.receiverId) {
        ElMessage.error('无法获取接单者信息，举报失败');
        reportLoading.value = false;
        return;
      }
      payload.reportedUserId = selectedOrderDetail.value.receiverId;
    } else if (payload.reportType === 'REVIEW') {
...
const submitReviewHandler = async () => {
    ...
    try {
        const payload = {
            ...
            revieweeId: selectedOrderDetail.value.receiverId,
